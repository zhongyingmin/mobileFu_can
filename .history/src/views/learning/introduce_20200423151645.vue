...
export default {
  ...
  methods: {
    ...
    init(div) {
      ...
      var div = document.createElement("div"); //创建<a>元素
      span.innerHTML = content.substring(0, len); //span里的内容为content的前len个字符

      div.innerHTML =
        content.length > len
          ? "<span style='font-size:14px;wight:100%;text-align:center'><i class='iconfont iconxia' style='font-size:18px'></i>展开全部</span>"
          : ""; ////判断显示的字数是否大于默认显示的字数    来设置a的显示
      div.style.cssText = `color: #666666`;
      ...
      div.onclick = function() {
        if (a.innerHTML.indexOf("展开") > 0) {
          //如果a中含有"展开"则显示"收起"
          a.innerHTML =
            "<span style='font-size:14px;wight:100%;text-align:center'><i class='iconfont iconxia' style='font-size:18px;display: -webkit-inline-box;transform: rotate(180deg);'></i>收起</span>";
