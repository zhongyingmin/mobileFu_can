    ...
    number() {
      ...
      return this.data.reduce((prev, next) => {
        ...
        if (!next.isSelected) {
          this.num--;
          return prev;
        } //如果商品是没有选中状态的，将直接返回prev，不计算本轮。
        return this.num++; //第一次的prev给默认指定是0，所以next是第一个商品，直接是第一个商品的数量乘价格。reduce每一轮循环返回的数值就是下一轮的prev。所以这对每一个选中的商品进行求和
      }, 0); //这个0是指定默认的第一次的prev是0
    }
  },
  mounted() {
