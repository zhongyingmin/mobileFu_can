    ...
    >
      <van-tab title="处方分析">
        <div class="p_chart">
          ...
          <div id="chats_c"></div>
          <div class="statistical" v-for="(item,index) in AList" :key="index">
            <div class="title-wrapper">
              <span class="nid">{{index+1}}.</span>
              <span class="name">{{item.name}}</span>
            </div>
            <div class="progtess-wrapper">
              <van-progress :percentage="item.percentage" stroke-width="12" :show-pivot="false" color="#c8a060" />
            </div>
            <div class="num-wrapper">{{item.number}}份</div>
          </div>
          <!-- <div id="chats_d"></div> -->
        </div>
      </van-tab>
...
export default {
  ...
  methods: {
    ...
    percentage() {
      ...
      for (let index = 0; index < this.AList.length; index++) {
        this.AList[index].percentage = this.AList[index].number / max;
      }
